...
$.fn.GridExtenPlus = function() {
    return this.each(function() {
        ...
        for (var k = 0; k < cellcount; k++) {
            ...
            //取3个表格对应列的outerWidth最大值 累加 作为table的宽度基本值
            taw += (a >= (b >= c ? b: c) ? a: (b >= c ? b: c));

            //取3个表格对应列的列宽最大的 作为列宽基本值
            a = thead.find("tr:first").find("td:eq(" + k + ")").width();
            b = tbody.find("tr:first").find("td:eq(" + k + ")").width();
            c = tfoot.find("tr:first").find("td:eq(" + k + ")").width();
            mw = a >= (b >= c ? b: c) ? a: (b >= c ? b: c);
            //不能用outerWidth设置td的width 所以用width的最大值设置
        ...
        //设置table宽度  +1的边框
        var tempw = taw + 1;
        thead.width(tempw).attr("basewidth", tempw);
        tbody.width(tempw).attr("basewidth", tempw);
        tfoot.width(tempw).attr("basewidth", tempw);
        //设置basewidth 外层div宽度18留给滚动
        tempw += 18;
        dhead.width(tempw).attr("basewidth", tempw);
        dbody.width(tempw).attr("basewidth", tempw);
        dfoot.width(tempw).attr("basewidth", tempw);
        ...
        //表格列宽拉伸事件
        thead.find("td:not('.zClum')").bind("mousemove",
        function(e) {
            var fx, fy;
            ...
            if (fx >= tdw - 5 && fx < tdw && fy > 0 && fy < tdh) {
                ...
                function(e) {
                    ...
                    thistd.closest(".GridExtenPlus").find("div.line").height(ah).show().offset({
                        top: ax,
                        left: sx
                    });
                    //拉伸
                    ...
                    function(e) {
                        //移动拉伸线
                        thistd.closest(".GridExtenPlus").find("div.line").offset({
                            top: ax,
                            left: e.pageX
                        });
                        //清除选择 防止部分浏览器第二次拖动时是选择状态
                        if ($.browser.mozilla || $.browser.safari || ($.browser.msie && $.browser.version >= 9)) window.getSelection().removeAllRanges();
                    }).bind("mouseup",
                    function(e) {
                        //改变后的理论宽度
                        ...
                        for (var k = 0; k < cellcount; k++) {
							...
							b = tbody.find("tr:first").find("td:eq(" + k + ")").outerWidth(true);
							c = tfoot.find("tr:first").find("td:eq(" + k + ")").outerWidth(true);
							//取3个表格对应列的outerWidth最大值 累加 做table的宽度校对
							taw += (a >= (b >= c ? b: c) ? a: (b >= c ? b: c));
							
							 //取3个表格对应列的列宽最大的 做列宽值校对
							a = thead.find("tr:first").find("td:eq(" + k + ")").width();
							b = tbody.find("tr:first").find("td:eq(" + k + ")").width();
							c = tfoot.find("tr:first").find("td:eq(" + k + ")").width();
							mw = a >= (b >= c ? b: c) ? a: (b >= c ? b: c);
							//校对td的width
							thead.find("tr:first").find("td:eq(" + k + ")").width(mw);
							tbody.find("tr:first").find("td:eq(" + k + ")").width(mw);
							tfoot.find("tr:first").find("td:eq(" + k + ")").width(mw);
                        }
        ...
        //行点击事件
        Grid.find("table.tbody tr").bind("click",
        function() {
            //限制域
        ...
        //console.log(thead.outerWidth()+"--"+tbody.width()+"--"+tfoot.width());
        if (tbody.height() > dbody.height()) {
            //thead.find("tr:first").find("td:last").width(thead.find("tr:first").find("td:last").width()+18);
            //tfoot.find("tr:first").find("td:last").width(tfoot.find("tr:first").find("td:last").width()+18);
        }
