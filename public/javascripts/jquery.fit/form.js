$.moduleLink = function (url, page, params, callback) {
    //待完善，应该成为所有链接的中间件
    var container = $('.AjaxPanel');
    if (container.size() == 0) window.location = url;
    else container.smartLoad({ url: url + "&layout=0", load: true, callback: function(){
                ...
                if (_confirm) {
                    if (!action && href) {  //只是链接
                        if (href) {
                            ...
                            $.moduleLink ( href, null, null, function(){
...
$.fn.ValidateFormat = function (url) {
    ...
    //console.log(this.find('.formatError'));
