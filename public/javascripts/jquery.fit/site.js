$.ReverseProxy = function (site, internalSite) { //消除网页内部一些 由 request.url 产生的 包含 localhost 的url，防止反向代理产生的问题
    if (site != "localhost")
        $('input[href]').each(function () {
            if (window.location.toString().indexOf(site) > 0) {
                var href;
                if (internalSite)
                    href = $(this).attr('href').replace(internalSite, site); //alert(href);
                else href = $(this).attr('href').replace(/localhost:\d+/, site); //alert(href);
                $(this).attr('href', href);
            }
        });
}
    ...
    return this.each(function () {
        ...
        if ((option.remove) && (option.remove == true)) {
            ...
            if ((option.insert) && (option.insert == true))
                ...
                var hold = { h: $(this).height(), w: $(this).width() }
                var margin = { l: $(this).css('margin-left'), r: $(this).css('margin-right') }
                $(this).hide();
                img.insertAfter(this).height(hold.h).width(hold.w).css('margin-left', margin.l).css('margin-right', margin.r);
            }
            else {
...
$.createDialog = function (option, onConfirm, loadCallback) {
    ...
    option.modal = true;
    option.width = option.width || document.documentElement.clientWidth * 0.8;
    option.height = option.height || document.documentElement.clientHeight * 0.8;
    ...
    dialog = $('<div ></div>').addClass('commonDialog').attr('title', option.title).appendTo('body').dialog(option).bind('confirm', function () {
        if (onConfirm($(this)))
            $(this).dialog('close');
    }).parent().addClass('cd_container').end();
...
$.fn.TableAutoWidth = function () {
    return this.each(function () {
        //把table 的宽度设置自动适应
        var grid = $(this);
        if (!grid.parent().hasClass('qg_container')) {
            var paginating = grid.next('.paginating');
            grid.wrap('<div class="qg_container" ></div>');
...
$.fn.DialogQueryParams = function () {
    return this.each(function () {
        var qp = $(this);
        $(this).find('[type=submit]').button().click(function () {
            ...
            $("#" + qp.attr('ajaxContainer')).indicator({ hide: true }).load(form.attr('action') + "?" + form.serialize(), function (data) {
                ...
                $(this).find('.QueryGrid').TableAutoWidth();
            });
            return false;
        });
    });
...
//初始化：  控件在生成html 之后是全部可用状态，接收到初始化命令就将部分设置为只读 (从$.ModulePage.config 取)
//          或者在生成html 的时候已经进行初始化
//状态切换：向控件发送命令，
// 1) toolbar 本身(负责所有编辑框)， 发送的参数不包含 state
// 2) 其它控件 (新增和修改功能), 发送的参数包含 state

$.fn.Toolbar_cmd = function (state) {
    return this.each(function () {
        var etf = $.ModulePage.config.table_edittypes();
        for (var i in etf) {
            $('[table="' + i + '"]').trigger('setDisable.Insert', [state && $.inArray(0, etf[i]) >= 0]);
            $('[table="' + i + '"]').trigger('setDisable.Delete', [state && $.inArray(2, etf[i]) >= 0]);
        }
    });
}
$.fn.Toolbar = function () {
    return this.each(function () {
        $(this).bind('setReadOnly', function (ui, init) {
            var rfs = $.ModulePage.config.readonly_fields();
            if (rfs) { //console.log(rfs);
                if (init) for (var i in rfs) { $('[fname=' + rfs[i] + ']').setReadOnly(true); }
                else $('[fname]').toggleReadOnly();
            }
            if (init) $(this).find('#btnSave').button().button('disable');
        });

        var newState = $(this).attr('newState');
        var btnedit = $(this).find("#btnEdit");
        if (newState) {
            setTimeout(function () {btnedit.button('disable');}, 1000);
        } else {
            //.trigger('setReadOnly', [true])
            btnedit.button().unbind('click').click(function () {
                $(this).trigger('setReadOnly', [false]);

                var toolbar = $(this).closest('.Toolbar');
                var toedit = $(this).val() == "修改";
                if (toedit) {
                    $(this).val("取消");
                    toolbar.find('#btnSave').button('enable');
                }
                else {
                    $(this).val("修改");
                    toolbar.find('#btnSave').button('disable');
                }

                $(this).Toolbar_cmd(toedit);
            }).end().trigger('setReadOnly', [false])/*.Toolbar_cmd(false)*/;
            
            $(function () { $(this).Toolbar_cmd(false); });
        }
    });
}
...
$.fn.DialogGrid = function (multisel) {
    return this.each(function () {
        $(this).Grid_Selectable(multisel);
        //$(this).width('2048px').width($(this).width());
    });
}

$.fn.Grid_Selectable = function (multisel) {
    return this.each(function () {
        if (multisel) {
            $('<th></th>').insertBefore($(this).find('thead tr th:first'));
            $('<td><input type="checkbox" /></td>').insertBefore($(this).find('tbody tr td:first-child'));
        }
        if (multisel) {
        ...
        else $(this).find('tbody tr').click(function () {
