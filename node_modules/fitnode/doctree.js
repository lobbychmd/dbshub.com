...
var doctree = function (db, config, expendNodes) {
    this.db = db;
    this.config = config;
    ...
    middleware: {
            ...
            dir: function (type, text, subtype, parent_attr, params) {
                ...
                for (c in letters) {
                    var l = { filter: (params && params.prefix ? params.prefix : "") + "[" + letters[c].toUpperCase() + letters[c].toLowerCase() + "]", text: letters[c], parent_type: type,
                        sub_type: subtype, middleware: 'az', nodeonly: true
                    };
        ...
        summary: {
            dir: function (type, text, subtype, parent_attr) {
                return [{ text: text, type: type, sub_type: subtype, middleware: 'summary', nodeonly: true}];
            },
            children: function (_doctree, parent_attr) {
                var query = {};
    ...
    fillQuery: function (parent_attr, query) {
        ...
        for (var q in query) {
            ...
            if (typeof r[q] == "string") {
                ...
                if (m) r[q] = r[q].replace(m[0], parent_attr[m[1]]);
            }
    ...
    get: function (attr, callback) {
        ...
        new new require('./util.js').seq_asyncArray(
                ...
                if (item.middleware) {
                    ...
                    if (item.type) {
                        ...
                        _doctree.db.collection(cc.table).find(_doctree.fillQuery(attr, item.query)).toArray(function (err, rows) {
                            for (var i in rows) {
                                var datarow = { text: rows[i][cc.text], type: item.type, table: cc.table, _id: rows[i]._id };
            ...
            function (params) {
                ...
                //递归调用
                if (params && (params.length > 0) && (this.expendNodes))
                    new new require('./util.js').seq_asyncArray(
                        function (item1, params1, callback1) {
                            var e = this.expendNodes[item1._id];
                            if (e)
